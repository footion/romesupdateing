...
public class R_ProductFrame extends basicFrame{
	...
	void inputData() {
		...
		nameList = new String[products.size()];
		keyList = new int[products.size()];
		for(int i=0;i<products.size();i++) {
			Product product = products.get(i);
			nameList[i]=product.getName();
			keyList[i]=product.getId();
		}
		dataList = new JList(nameList);
		scrollPane.setViewportView(dataList);
	}
	public int returnDataKey() {
		System.out.println(dataList.getSelectedIndex());
		System.out.println(keyList.length);
		int Key = keyList[dataList.getSelectedIndex()];
		return Key;
	}
